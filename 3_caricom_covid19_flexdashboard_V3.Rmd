...
### Total Confirmed Cases in CARICOM
...
### Total Deaths in CARICOM
```{r}
valueBox(caricom_totals$total_deaths, 
         icon = "fa-skull-crossbones")
```

### Total Recoveries in CARICOM
```{r}
valueBox(caricom_totals$total_recovered, 
...
```{r}
library(ggplot2)
deaths_boxplot <- ggplot(world_covid_regression_data, 
                     aes(x=factor(income), 
                         y=deaths_per_100k)) +
  geom_boxplot() + 
...
### Relationship Between Confirmed Cases per 100k (Log Scale) and Population Aged 65 +
```{r}
...
### Relationship Between Confirmed Deaths per 100k (Log Scale) and Population Aged 65 +

```{r}
regression_bi_ggplot <- world_today %>%
  ggplot(aes(pop_65_over_2018, log(deaths_per_100k))) +
  geom_point() + 
  stat_smooth(method = "lm") + 
  labs(x = "Population Aged 65+",
       y = "Deaths per 100,000 Persons (Log Scale)")
library(plotly)
ggplotly(regression_bi_ggplot)
```
